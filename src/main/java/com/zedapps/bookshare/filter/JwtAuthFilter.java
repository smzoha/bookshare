...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        String token = authHeader.substring(7); // trimming out "Bearer "
        String email = jwtService.getEmail(token);

        if (StringUtils.isNotBlank(email) && SecurityContextHolder.getContext().getAuthentication() == null) {
            LoginDetails loginDetails = (LoginDetails) loginDetailService.loadUserByUsername(email);

            if (jwtService.isTokenValid(token, loginDetails)) {
